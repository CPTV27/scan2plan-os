...
- Schema defined in `shared/schema/db.ts` (exports all tables)
- Schema entry point: `shared/schema.ts` (re-exports from db.ts)
...
- `server/storage/leads.ts` - Lead and document operations
- `server/storage/quotes.ts` - CPQ quotes and versions
- `server/storage/projects.ts` - Production projects
- `server/storage/users.ts` - User management
- `server/storage/financial.ts` - Invoices, accounts, loans
